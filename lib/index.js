...
module.exports = class {
    ...
    // uploader all assets inside 'root', that match 'pattern'
    // optionally writing the manifest to 'outputManifestPath'
    * upload(root, pattern, outputManifestPath) {
        var files = yield this._findAssets(root, pattern)
        var manifest = yield Promise.map(files, (path) => this._uploadAsset(root, path))
    ...
    // upload an individual asset to the configured provider
    _uploadAsset(root, absolutePath) {
        ...
        return Promise.coroutine(function*() {
            ...
            yield self.provider.upload(key, hash, readStream, contentType)
